...
def update_b(H,A,pol):
	...
	for p in pol.T:  # Iterate over each action (columns of pol0)
		...
		# b += temp * p[:, np.newaxis] # Multiply by the policy
...
def update_H(H, A ,E , pol0, pol):
	...
	if cols > 1: pol = matop.normalize_rows(pol+0.001) # 0.001 as nonzero hack for Gmatrix convergence

    ### Routine to update H based on new policy ###

	### Step 1. Remove the known impact from the sim
	b0 = update_b(H,A,pol0)
