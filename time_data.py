...
def wall_clock_test(n,m):
	...
	for i in range(1,n+1): # Itarate over number of robots
		for j in range(0,m): # Do multiple runs to average out
			tic = time.time()    
			f = sim.run(i,i) # Launch simulation run
			toc = time.time()
			t[i-1,j] = toc-tic
...
def run(n,m,batch,filename=None):
	...
	# Save
	if filename is not None: np.savez(filename, t_animation=t_animation,t=t,t_batch=t_batch)
...
def load(filename):
	...
	return t, t_animation, t_batch

def plot(filename,figurename=None):
	t, t_animation, t_batch = load(filename)
	...
	plt.plot(range(1,t_batch.shape[0]+1),t_batch.mean(axis=1),label="t_batch")
